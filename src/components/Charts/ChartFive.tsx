...
import { ChartFiveState } from '../../interfaces/ChartFive';
  ...
  const fetchData = async () => {
    try {
      ...
      if (Array.isArray(result)) {
        ...
        result.forEach((item) => {
          const { avg_latency, local_isp } = item;

          if (
            local_isp.toLowerCase().includes('viettel') &&
            viettelCount <= 20
          ) {
            viettelData.push([viettelCount++, avg_latency]);
          } else if (
            local_isp.toLowerCase().includes('vnpt') &&
            vnptCount <= 20
          ) {
            vnptData.push([vnptCount++, avg_latency]);
          } else if (
            local_isp.toLowerCase().includes('fpt') &&
            fptCount <= 20
          ) {
